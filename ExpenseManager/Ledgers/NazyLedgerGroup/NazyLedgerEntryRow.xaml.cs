...
namespace ExpenseManager.Ledgers.NazyLedgerGroup
{
    ...
    /// </summary>
    public partial class NazyLedgerEntryRow : UserControl
    {
        readonly MoneyLedger moneyLedger;
        readonly int runningBalance;

        public NazyLedgerEntryRow(MoneyLedger moneyLedger, int runningBalance)
        {
            InitializeComponent();
            ...
            else
            {
                ...
                HelperMethods.AskYesNo(async () =>
                {
                    ...
                    else if (moneyLedger.RefType == "Invoice")
                    {
                        ...
                        {
                            await Task.Run(() => MainWindow.NazyInvoiceManager.BatchDeleteData(invoices.Select(i => i.ID).ToList()));
                            await Task.Run(() => MainWindow.MoneyLedgerManager.RemoveData(moneyLedger.ID));
                        }
                    }
                }, message);
