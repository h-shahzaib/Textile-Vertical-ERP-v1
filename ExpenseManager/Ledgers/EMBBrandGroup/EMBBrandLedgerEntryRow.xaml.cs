...
namespace ExpenseManager.Ledgers.EMBBrandGroup
{
    ...
    /// </summary>
    public partial class EMBBrandLedgerEntryRow : UserControl
    {
        EMBBrandLedger ledger;

        public EMBBrandLedgerEntryRow(EMBBrandLedger ledger)
        {
            InitializeComponent();
            this.ledger = ledger;
        ...
        private void DeleteRow()
        {
            if (ledger.InvGroupID == -1)
                HelperMethods.AskYesNo(async () =>
                   await MainWindow.EMBBrandLedgerManager.RemoveData(ledger.ID));
            else
        ...
        private void DeleteInvoice(int GroupID)
        {
            ...
            {
                await MainWindow.EMBBrandLedgerManager.RemoveData(ledger.ID);
                foreach (var item in MainWindow.rawDataManager.EMBInvoices.Where(i => i.Brand == ledger.Brand && i.GroupID == ledger.InvGroupID))
                    await MainWindow.EMBInvoiceManager.RemoveData(item.ID);
            }, s);
        }
        ...
        private void Ledger_Detail_Row_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            var runningBal = MainWindow.rawDataManager.EMBBrandLedgers
