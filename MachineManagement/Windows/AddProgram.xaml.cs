                    ...
                    foreach (SqliteDataAccess.Stock stock in EntryNumFilteredStock)
                    {
                        ...
                        foreach (string RawColor in RawColors)
                        {
                            ...
                            foreach (string LotColorStr in stock.RepString.Split(',').Where(i => i.Split('-')[0] == RawColor))
                            {
                                ...
                                if (DesignsMany)
                                {
                                    SqliteDataAccess.Design Design = MainWindow.rawDataManager.Designs.Where(i => i.ID.ToString() == LotColorStr.Split('-')[1]).FirstOrDefault();
                                    ...
                                    foreach (SqliteDataAccess.MchStock mchStock in MainWindow.rawDataManager.MchStocks.Where(i => i.StockID == stock.DiaryNumber))
                                    {
                                        ...
                                        foreach (string split in splits.Where(i => i.Split('-')[1] == Design.ID.ToString()))
                                    ...
                                    if (usedReps != total)
                                    {
                                        AccessoryCard accessorycard = new AccessoryCard(ThreadColors, ExtraAccs, usedReps, total, Design.ID.ToString());
                                        accessorycards.Add(accessorycard);
                                    }
                                }
                                else
                                {
                                    SqliteDataAccess.Design Design = MainWindow.rawDataManager.Designs.Where(i => i.ID.ToString() == stock.DesignId.ToString()).FirstOrDefault();
                        ...
                        {
                            SqliteDataAccess.Design OrignalDesign = MainWindow.rawDataManager.Designs.Where(d => d.ID == stock.DesignId).FirstOrDefault();
